...
function draw() {
  ...
  if (checkMouse()) {
    ...
    x = width / 2 + 1.5 * R * sin(frameCount * 0.05);
    y = height / 2 + R * cos(frameCount * 0.05);
    //x=lerp(x,,0.6);
    //y=lerp(y,map(cos(frameCount*0.05),-1,1,height/2-100,height/2+100),0.6);
    r = 220;
    p = random(0.98, 1.02);
...
function drawNoiseBackground() {
  ...
  for (let y = 0; y < height; y += 10) {
    let c = map(sin(t + y * 0.02), -1, 1, 0, 80);
    fill(c);
    rect(0, y, width, 10);
  }
}
...
function drawCreature(x, y) {
  ...
  for (let a = 0; a < TWO_PI; a += PI / 8) {
    if (mouseIsPressed) {
      ...
      currentH = 150;
      fill(
        map(noise(frameCount * 0.02), 0, 1, 130, 190),
        map(noise(frameCount * 0.02), -1, 1, 70, 140),
        map(sin(frameCount * 0.01), -1, 1, 100, 170)
      );
    }
  ...
  fill(
    r,
    map(cos(frameCount * 0.03), -1, 1, 130, 140),
    map(noise(frameCount * 0.02), 0, 1, 120, 130)
  );
  circle(0, 0, coreR);
  fill(0);
